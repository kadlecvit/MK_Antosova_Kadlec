...
def load_polygon_points(shp_path):
    # Load polygon boundary points from shapefile
...
def reduce_to_convex_hull(points_geo):
    # Reduce point set to convex hull vertices
...
def point_in_circle(pt, center, radius, tol=1e-10):
    # Check if point lies inside a circle
    return np.linalg.norm(np.array(pt) - center) <= radius + tol


def welzl(pts, boundary, n):
    # Welzl's recursive algorithm for smallest enclosing circle
...
def solve_sec(points_geo):
    # Solve smallest enclosing circle for a set of points
...
def run_stereo(shp_path):
    ...
    # Find cartographic pole K as center of smallest enclosing circle
    uk, vk, psi_j_deg = solve_sec(hull_pts)
    psi_j = radians(psi_j_deg)
    s_j   = pi/2 - psi_j

    # Multiplicative constant for equal distortion at pole and boundary
    ...
    # Scale factors at pole, boundary, and undistorted parallel
    m_pol = mju / cos(0)**2
    m1    = mju / cos(psi_j/2)**2
    m0    = mju / cos(psi0/2)**2

    # Length distortions in mm/km
    ny_pol = (m_pol-1)*1000
    ny1    = (m1-1)*1000
    ny0    = (m0-1)*1000
...
def uv_sd(u, v, uk_r, vk_r):
    # Transform geographic coordinates to oblique cartographic coordinates (s, d)
...
def plot(shp_path, uk, vk, mju, psi_j, psi0, ny1, ny_pol, ny0):
    ...
    # Transform boundary to map coordinates (stereographic projection)
    ...
    # Compute distortion field on a regular grid (circles centred at K)
    mg = 0.01
    ...
    ax.set_aspect('equal')
    ax.set_title(f'Stereografická projekce\nν = ±{ny_bnd:.4f} mm/km')
